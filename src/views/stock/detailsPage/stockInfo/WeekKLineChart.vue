<template>
    <div>
      ...
      <div id="weekKLineChart" class="weekKLineChart" style="width: 960px;height: 360px"></div>
    ...
    export default {
      ...
      methods: {
        initChartSetMethod(rawData) {
          ...
          let weekKLineChart = echarts.init(document.getElementById('weekKLineChart'));
          ...
          // 使用刚指定的配置项和数据显示图表
          weekKLineChart.setOption(optionWeekKLine);

        },

        //获取股票K线图
        getKLineDataMethod() {
          return getKLineData(this.code, 'week').then(response => {
            if (response) { // 如果返回有信息
